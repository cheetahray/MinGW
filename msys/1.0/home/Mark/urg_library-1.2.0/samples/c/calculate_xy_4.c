    ...
    */
    ...
    while(1)
    {
            ...
            */
            ...
            if(ghost++ > 7)
            {
                ...
                if( 1 == keypress )
                {
                    keypress = 2;
#if 0 //def littlestar
                    if ( lo_send(t, "/radar", "iii", 4, aluanX, 196 ) == -1 )
#else
                    if ( lo_send(t, "/radar", "iii", 4, aluanX, aluanY ) == -1 )
#endif
                        printf("OSC error %d: %s\n", lo_address_errno(t), lo_address_errstr(t));
                    else if(0)
                        printf("%ld ,%ld\n", aluanX, aluanY);
					if ( lo_send(t1, "/button1", "iff", 4, outputMatrix[0][0], outputMatrix[0][1] ) == -1 )
                        printf("OSC error %d: %s\n", lo_address_errno(t), lo_address_errstr(t));
        ...
        else
        {
            ...
            {
#if 1 //def littlestar
                if ( lo_send(t, "/radar", "iii", 4, 0, 0 ) == -1 )
                    printf("OSC error %d: %s\n", lo_address_errno(t), lo_address_errstr(t));
                else if(1)
                    printf("%ld ,%ld\n", 0, 0);
#endif
